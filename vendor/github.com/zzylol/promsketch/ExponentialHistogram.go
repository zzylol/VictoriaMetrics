...
func (ehdd *ExpoHistogramDD) GetMemory() float64 {
	var total_mem float64 = 0
	for i := 0; i < ehdd.s_count; i++ {
		total_mem += float64(unsafe.Sizeof(*ehdd.dd[i]))
	}
...
func (ehdd *ExpoHistogramDD) QueryIntervalMergeDD(t1, t2 int64) *ddsketch.DDSketch {
	...
	var from_bucket, to_bucket int = 0, 0
	for i := 0; i < ehdd.s_count; i++ {
		if t1 >= ehdd.min_time[i] && t1 <= ehdd.max_time[i] {
			from_bucket = i
		}
		if t2 >= ehdd.min_time[i] && t2 <= ehdd.max_time[i] {
			to_bucket = i
		}
		/*
			curdiff1 := AbsInt64(t1 - ehdd.min_time[i])
			curdiff2 := AbsInt64(t2 - ehdd.max_time[i])
			if curdiff1 < diff1 {
				diff1 = curdiff1
				from_bucket = i
			}
...
func (eh *ExpoHistogramCount) Update(time int64, value float64) {
	...
	for i := eh.s_count - 2; i >= 0; i-- {
		if eh.buckets[i].bucketsize == eh.buckets[i+1].bucketsize {
			...
			if float64(same_size_bucket) >= float64(eh.k)/2.0+2 {
				...
				eh.buckets[i+1].sum += eh.buckets[i+2].sum
				eh.buckets[i+1].sum2 += eh.buckets[i+2].sum2
				eh.buckets[i+1].bucketsize += eh.buckets[i+2].bucketsize
				eh.buckets[i+1].max_time = MaxInt64(eh.buckets[i+1].max_time, eh.buckets[i+2].max_time)
				eh.buckets[i+1].min_time = MinInt64(eh.buckets[i+1].min_time, eh.buckets[i+2].min_time)
	...
	if float64(same_size_bucket) >= float64(eh.k)/2.0+2 {
		...
		eh.buckets[0].sum += eh.buckets[1].sum
		eh.buckets[0].sum2 += eh.buckets[1].sum2
		eh.buckets[0].bucketsize += eh.buckets[1].bucketsize
		eh.buckets[0].max_time = MaxInt64(eh.buckets[0].max_time, eh.buckets[1].max_time)
		eh.buckets[0].min_time = MinInt64(eh.buckets[0].min_time, eh.buckets[1].min_time)
