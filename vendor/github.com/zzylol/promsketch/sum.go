...
func NewEfficientSum(item_window_size int64, time_window_size int64, epsilon float64, R float64) *EfficientSum {
	s := EfficientSum{
		...
		i:                0,
		m:                0,
		v:                math.Ceil(math.Log2(1 / epsilon * math.Log2(float64(item_window_size)))),
	}
	s.T = float64(item_window_size) * (2*epsilon - math.Pow(2, -s.v))
	if epsilon*float64(item_window_size)*(2-1/math.Log2(float64(item_window_size))) >= 2 {
		s.algo = LargeError
...
func (s *EfficientSum) GetMemory() float64 {
	return (float64(len(s.b)) + 73) / 1000 // KB
}

func (s *EfficientSum) Cover(mint, maxt int64) bool {
	// fmt.Println("mint, maxt, max_time, windowsize", mint, maxt, s.b[s.i].max_time, s.time_window_size)
	return mint >= s.b[s.i].max_time-s.time_window_size // && maxt <= s.b[s.i].max_time
}
...
func (s *EfficientSum) QueryLargeError(t1, t2 int64, subinterval bool) float64 {
	...
	sum_B += int64(s.b[idx].bit)
	return s.R * (float64(s.T)*float64(sum_B) - s.T + math.Pow(2, -s.v-1)) // TODO: fix s.m
}
...
func (s *EfficientSum) QuerySmallError(t1, t2 int64, subinterval bool) float64 {
	...
	sum_B += int64(s.b[idx].bit)
	return s.R * (float64(s.T)*float64(s.B) - s.T + math.Pow(2, -s.v-1))
}
