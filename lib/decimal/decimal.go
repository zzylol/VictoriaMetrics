...
import (
	...
	"github.com/zzylol/VictoriaMetrics/lib/fastnum"
...
func positiveFloatToDecimalSlow(f float64) (int64, int16) {
	...
	if f > 1e6 || f < 1e-6 {
		...
		if f > 1e6 {
			...
			// See https://github.com/zzylol/VictoriaMetrics/issues/213
			prec = 1e15
		}
		_, exp := math.Frexp(f)
		// Bound the exponent according to https://en.wikipedia.org/wiki/Double-precision_floating-point_format
		// This fixes the issue https://github.com/zzylol/VictoriaMetrics/issues/1114
		if exp < -1022 {
			exp = -1022
		} else if exp > 1023 {
			exp = 1023
