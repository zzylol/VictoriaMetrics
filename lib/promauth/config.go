...
// See https://github.com/zzylol/VictoriaMetrics/issues/1764
type Secret struct {
	S string
}
...
func (ac *Config) SetHeaders(req *http.Request, setAuthHeader bool) error {
	if ac.tlsServerName != "" {
		...
		// See https://github.com/zzylol/VictoriaMetrics/pull/5802
		req.Host = ac.tlsServerName
	}
	reqHeaders := req.Header
	for _, h := range ac.headers {
		if h.key == "Host" {
			// Host header must be set via req.Host - see https://github.com/zzylol/VictoriaMetrics/issues/5969
			req.Host = h.value
		} else {
...
func (tctx *tlsContext) initFromTLSConfig(baseDir string, tc *TLSConfig) error {
	...
	if len(tc.Key) != 0 || len(tc.Cert) != 0 {
		...
		tctx.getTLSCert = func(*tls.CertificateRequestInfo) (*tls.Certificate, error) {
			// Re-read TLS certificate from disk. This is needed for https://github.com/zzylol/VictoriaMetrics/issues/1420
