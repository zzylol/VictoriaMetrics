...
import (
	...
	"github.com/zzylol/VictoriaMetrics/lib/leveledbytebufferpool"
	...
	parser "github.com/zzylol/VictoriaMetrics/lib/protoparser/prometheus"
	"github.com/zzylol/VictoriaMetrics/lib/protoparser/prometheus/stream"
...
func (sw *scrapeWork) run(stopCh <-chan struct{}, globalStopCh <-chan struct{}) {
	...
	if scrapeAlignInterval <= 0 {
		...
		// See https://github.com/zzylol/VictoriaMetrics/issues/2679
		//
	...
	for {
		...
		case <-stopCh:
			...
			case <-globalStopCh:
				...
				// See https://github.com/zzylol/VictoriaMetrics/issues/2013#issuecomment-1006994079
			default:
...
func (sw *scrapeWork) processDataInStreamMode(scrapeTimestamp, realTimestamp int64, body *bytesutil.ByteBuffer, scrapeDurationSeconds float64) error {
	...
	err := stream.Parse(r, scrapeTimestamp, false, false, func(rows []parser.Row) error {
		...
		// See https://github.com/zzylol/VictoriaMetrics/issues/825#issuecomment-723198247
		sw.pushData(sw.Config.AuthToken, &wc.writeRequest)
		wc.resetNoRows()
		return nil
	}, sw.logError)
...
func (sw *scrapeWork) areIdenticalSeries(prevData, currData string) bool {
	if sw.Config.NoStaleMarkers && sw.Config.SeriesLimit <= 0 {
		...
		// The check for series_limit is needed for https://github.com/zzylol/VictoriaMetrics/issues/3660
		return true
	}
...
func (sw *scrapeWork) sendStaleSeries(lastScrape, currScrape string, timestamp int64, addAutoSeries bool) {
	...
	// See https://github.com/zzylol/VictoriaMetrics/issues/3668
	...
	if bodyString != "" {
		...
		// See https://github.com/zzylol/VictoriaMetrics/issues/3668
		// and https://github.com/zzylol/VictoriaMetrics/issues/3675
		var mu sync.Mutex
		...
		err := stream.Parse(br, timestamp, false, false, func(rows []parser.Row) error {
			...
			// See https://github.com/zzylol/VictoriaMetrics/issues/3660
...
func (sw *scrapeWork) addRowToTimeseries(wc *writeRequestCtx, r *parser.Row, timestamp int64, needRelabel bool) {
	...
	//
	// See https://github.com/zzylol/VictoriaMetrics/issues/3557
	// and https://github.com/zzylol/VictoriaMetrics/issues/3406
