...
var transformFuncs = map[string]transformFunc{
	...
	"time":                       transformTime,
	// "timestamp" has been moved to rollup funcs. See https://github.com/zzylol/VictoriaMetrics/issues/415
	...
	for _, xss := range m {
		...
		for _, xs := range xss {
			...
			if isZeroTS(ts) {
				...
				// for zero buckets.
				// See https://github.com/zzylol/VictoriaMetrics/pull/4021
				continue
			}
...
func fixBrokenBuckets(i int, xss []leTimeseries) {
	...
	// See https://github.com/zzylol/VictoriaMetrics/issues/4580#issuecomment-2186659102
	if len(xss) < 2 {
		return
	}
