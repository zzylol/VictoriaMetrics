...
var (
	...
	queryTimeAlignment = flag.Bool("datasource.queryTimeAlignment", true, `Deprecated: please use "eval_alignment" in rule group instead. `+
		...
		"See more details at https://github.com/zzylol/VictoriaMetrics/pull/1257")
...
func Init(extraParams url.Values) (QuerierBuilder, error) {
	...
	if *lookBack != 0 {
		logger.Warnf("flag `-datasource.lookback` is deprecated and will be removed in next releases. Please adjust `-search.latencyOffset` at datasource side or specify `latency_offset` in rule group's params. See https://github.com/zzylol/VictoriaMetrics/issues/5155 for details.")
	}
